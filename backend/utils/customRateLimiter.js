...
const customRateLimiter = (req, res, next) => {
    ...
    const attempts = global.ipAttempts[deviceId] || { count: 0, lastAttempt: 0, bannedUntil: null };
  
    
    // Check if the deviceId is banned
    if (attempts.bannedUntil && currentTime < attempts.bannedUntil) {
      return res.status(403).json({ message: 'Your device is temporarily banned due to repeated failed attempts.' });
    }
    ...
    if (attempts.count >= 4) {
      ...
      return res.status(403).json({ message: 'Too many failed attempts, your device is temporarily banned.' });
    }
    ...
    if (attempts.count > 3) {
      return res.status(429).json({
        message: `Too many requests. Please try again after ${Math.ceil(cooldownTime / 1000 / 60)} minutes.`,
